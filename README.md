...
└─ app/
   ...
   ├─ run_python_task.sh   # 为 cron 恢复任务所需的容器环境变量
   ├─ task_lock.py         # 防止挂机、对话等浏览器任务并发冲突
...
- `03:00、20:00`：AI 对话积分任务。
- `04:00、06:00`：云电脑使用时长积分任务。
- 同一时间只运行一个浏览器任务；如果前一个任务尚未结束，后一个任务会安全跳过，避免登录态和积分数据包互相干扰。
