...
configure_cron_in_container() {
    ...
    docker exec -i "$container_name" sh -c "cat > /etc/cron.d/ctyun-cron && chmod 0644 /etc/cron.d/ctyun-cron" <<EOF
${login_cron} root /app/run_python_task.sh ${login_script} > /proc/1/fd/1 2>&1
${pc_cron} root /app/run_python_task.sh ${pc_script} > /proc/1/fd/1 2>&1
EOF

    docker exec "$container_name" service cron reload >/dev/null 2>&1 || true
...
echo -e "查看定时任务：docker exec -it ${CONTAINER_NAME} cat /etc/cron.d/ctyun-cron"
echo -e "AI 定时日志：docker exec -it ${CONTAINER_NAME} tail -n 100 /app/data/cron_login_script.log"
echo -e "云电脑定时日志：docker exec -it ${CONTAINER_NAME} tail -n 100 /app/data/cron_pc_login.log"
echo -e "------------------------------------------------------"
